...
export class MediaStreamHandler {
        ...
        connection.on("message", async (message) => {
            await this.handleTwilioMessage(
                ...
                (resolvedSessionId, resolvedSession) => {
                    ...
                    // Initialize OpenAI on first session resolution
                    if (!openAIWs && session) {
                        this.initializeOpenAIForSession(session, connection);
                    }
                },
            );
        });
    ...
    async initializeMCPComponents(session) {
        try {
            ...
            console.log(
                "✅ MCP and AI components initialized for session:",
                session.id,
            );
        } catch (error) {
    ...
    async handleTwilioMessage(message, connection, onSessionResolved) {
        try {
            const data = JSON.parse(message);

            if (data.event === "start") {
                // Get session ID from start event (this is where Twilio sends the CallSid)
                ...
                // Find the existing session created by HTTP handler
                const resolvedSession = this.sessionManager.getSession(callSid);

                if (!resolvedSession) {
                    console.error(
                        `❌ Session ${callSid} not found! Creating fallback session.`,
                    );
                    // Create fallback session if needed
                    const fallbackSession = this.sessionManager.createSession(
                        callSid,
                        "Unknown",
                        {},
                    );
                ...
                // Handle media after session is resolved
                // This will be handled by the OpenAI message handler
            }
        } catch (error) {
    ...
    async initializeOpenAIForSession(session, connection) {
        console.log(
            `📋 Initializing OpenAI for session ${session.id} - Caller: ${session.callerNumber}`,
        );
        ...
        // Store openAIWs reference for media handling
        connection.openAIWs = openAIWs;

        // Set up media handling after OpenAI is ready
        connection.on("message", (message) => {
            this.handleMediaMessage(message, session, openAIWs);
        });
    }

    handleMediaMessage(message, session, openAIWs) {
        try {
            const data = JSON.parse(message);
    ...
    handleCallStart(data, session, openAIWs) {
        ...
        const firstMessage =
            customParameters?.firstMessage || session.firstMessage;
    ...
    async handleFunctionCall(response, session, openAIWs) {
        ...
        if (session.aiOrchestrator && session.mcpClient) {
            ...
            console.error(
                "⚠️ MCP not available, sending fallback for function call:",
                functionName,
            );
            const fallbackOutput = {
                ...
                item: {
                    ...
                    output: JSON.stringify({
                        success: false,
                        error: "Hospital system temporarily unavailable. Please try again.",
                    }),
                },
            };
            ...
            openAIWs.send(
                JSON.stringify({
                    ...
                    response: {
                        modalities: ["text", "audio"],
                        instructions:
                            "Apologize to the patient that the hospital system is temporarily unavailable and ask them to try again in a moment.",
                    },
                }),
            );
        }
